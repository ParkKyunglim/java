...
update register set regMidScore = 90 ,regFinalScore = 80 where stdno ='22421003' 
 60,30
 54,36
 52,28
 38,40
...
#문제7. 수강 테이블에 출석점수, 중간고사점수, 기말고사 점수를 모두 더한 평균을 구해 총점에 입력하시오
 ...
 #문제8. 수강 테이블에 총점을 기준으로 A~F 등급을 입력하시오
  select  
    ...
    `regFinalScore`,
    (regAttenScore +regmidscore + regfinalscore)/3 as '총합' , 
   case when (regAttenScore +regmidscore + regfinalscore)/3 >= 40 then 'A'
        when (regAttenScore +regmidscore + regfinalscore)/3 >= 35 then 'B'
        when (regAttenScore +regmidscore + regfinalscore)/3 >= 30 then 'C'
        when (regAttenScore +regmidscore + regfinalscore)/3 >= 25 then 'D'
        else 'F'
    end as `등급`
   from register;
...
select b.stdNO, b.stdName, c.lecNo , c.lecName, d.proNo, d.proName, a.regAttenScore , a.regMidScore , a.regFinalScore , (regAttenScore +regmidscore + regfinalscore)/3 as `평균`  
  from register a
  left join student b
    on a.stdNo = b.stdNO 
  left join lectures c
